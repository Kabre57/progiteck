    ...
    for (const [roleName, permissionStrings] of Object.entries(rolePermissions)) {
      ...
      if (role) {
        ...
        for (const permString of permissionStrings) {
          ...
          // Vérification que resource et action sont définis
          if (!resource || !action) {
            console.log(`   ⚠️ Format de permission invalide: ${permString}`);
            continue;
          }

          // Trouver la permission dans la liste déjà chargée (évite le problème de where)
